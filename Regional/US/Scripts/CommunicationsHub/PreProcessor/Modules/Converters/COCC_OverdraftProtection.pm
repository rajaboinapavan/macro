package Converters::COCC_OverdraftProtection;
...
# Module Globals
#
my $DOCUMENT_TYPE   = q(OverdraftProtection);    # used for output JSON file postfix
my $FILENAME_SUFFIX = q(MM_OPTR_NOTICE);         # used for input XML file postfix
...
sub handle_document_details {
	...
	my $doc_details = setCOCCDocumentDetails(
		$set,
		{
			'ACCOUNT_TRANSFER_DETAILS' => {
				'FROM_ACCOUNT' => {
					'ACCOUNT_NUMBER'  => 'Account_Information/From_AcctNbr',
					'TRANSFER_AMOUNT' => 'Account_Information/From_Tran_Amt',
				},
				'TO_ACCOUNT' => {
					'ACCOUNT_NUMBER' => 'Account_Information/To_AcctNbr',
				},
			},
			'AVAILABLE_BALANCE' => 'Account_Information/Avail_Balance',
			'CURRENT_BALANCE'   => 'Account_Information/Current_Balance',
			'TRANSFER_FEE'      => 'Account_Information/Fee_Amt',
	...
	$doc_details->{'TEMPLATE'}      = 'Overdraft Protection Notice';
